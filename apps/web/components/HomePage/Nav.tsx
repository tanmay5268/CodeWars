...
const Nav = () => {
  ...
  return (
    ...
    <header className=" overflow-hidden w-full bg-neutral-950 flex justify-between items-center px-6 h-16 border-none">
      ...
      <nav className="flex uppercase justify-center items-center-safe font-[Space] h-10 text-white w-80 gap-7">
        {!session?.user ?<button className="font-bold border-2 hover:rounded-none rounded-lg px-2 py-1 hover:scale-105 transition-all ease-in-out duration-300 hover:text-[#9cff93] text-slate-500" onClick={handleSignIn}>
          SIGN IN
        </button> : <button className="font-bold border-2 hover:rounded-none rounded-lg px-2 py-1 hover:scale-105 transition-all ease-in-out duration-300 hover:text-[#9cff93] text-slate-500" onClick={() => signOut()}>
          SIGN OUT
        </button> }
